...
export type NodeId =
  ...
  | 'gladiator_utility.soaring_reach' | 'gladiator_utility.momentum'
...
export type SkillTree = 'fire' | 'lightning' | 'frost' | 'utility' | 'archer' | 'archer_utility' | 'arms' | 'bulwark' | 'hunter' | 'gladiator_utility';
...
export const GATES: Partial<Record<NodeId, Gate>> = {
  ...
  'arms.spear_flurry':    { requiresAll: ['arms.spear_throw'], requiresAny: ['arms.stunning_blow', 'arms.serrated_edge'] },
  ...
  // Footwork utility tree — leap is the ungated starter of its own tree.
  'arms.crushing_landing':{ requiresAll: ['arms.leap'] },
  'gladiator_utility.soaring_reach': { requiresAll: ['arms.leap'] },
  'gladiator_utility.momentum':      { requiresAll: ['arms.leap'], requiresAny: ['arms.crushing_landing', 'gladiator_utility.soaring_reach'] },
  // Bulwark tree
...
export const SKILL_NODES: SkillNode[] = [
  ...
  // id keeps its historical 'arms.' prefix — saved skill_unlocks and item affixes reference it; tree membership is the data that moved.
  { id: 'arms.leap',           name: 'Leap',           tree: 'gladiator_utility', tier: 1, cost: 1, isSpell: true,  description: 'Leap to a point. Enemies at the landing are slowed.' },
  // id keeps its historical 'arms.' prefix — saved skill_unlocks and item affixes reference it; tree membership is the data that moved.
  { id: 'arms.crushing_landing', name: 'Crushing Landing', tree: 'gladiator_utility', tier: 2, cost: 2, isSpell: false, description: 'Stronger landing slow per rank.', stackable: { softCap: 3, baseEffect: 0.10 },
  ...
  { id: 'arms.spear_flurry',    name: 'Spear Flurry',    tree: 'arms', tier: 4, cost: 2, isSpell: true,  description: 'A 1s burst of 5 cone thrusts at your cursor. 30–45 each.' },
  { id: 'arms.extended_flurry', name: 'Extended Flurry', tree: 'arms', tier: 5, cost: 1, isSpell: false, description: '+1 flurry hit per rank.', stackable: { softCap: 3, baseEffect: 1 },
    keystone: { name: 'Bloodsong', description: 'Landing every flurry hit on one target stuns them for 0.5s.' } },
  { id: 'arms.harpoon',         name: 'Harpoon',         tree: 'arms', tier: 5, cost: 3, isSpell: true,  description: 'Skillshot that drags the victim to melee range. 70–90 damage.' },
  { id: 'arms.quick_reel',      name: 'Quick Reel',      tree: 'arms', tier: 6, cost: 1, isSpell: false, description: 'Harpoon cooldown reduced per rank.', stackable: { softCap: 3, baseEffect: 0.10 },
  ...
  // Footwork utility tree — Leap and Crushing Landing live here (as arms.* ids, above).
...
export const SPELL_BINDINGS: SpellBinding[] = [
  ...
  { spell: 21, node: 'bulwark.war_cry',      charClass: 'gladiator' },
  { spell: 22, node: 'arms.harpoon',         charClass: 'gladiator' },
  { spell: 23, node: 'bulwark.kick_up_dust', charClass: 'gladiator' },
];
