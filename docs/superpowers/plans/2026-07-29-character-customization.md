...
- Left column: cycle-style pickers (`◀ value ▶` rows, `.px-btn` styling to match the pixel theme kit) for: Body, Skin, Hair Style, Hair Color, Eyes, Shirt Color, Pants Color — options straight from `APPEARANCE_OPTIONS`; a `⚄ Randomize` button calls `randomAppearance`.
